...
survey <- survey_raw %>% 
  ...
  mutate(capacity = factor(capacity, levels = c("Low capacity", "High capacity")))
```
...
department_plot <- department %>% 
  ...
  scale_y_continuous(limits = c(0, 0.37), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  theme(panel.grid.minor.x = element_blank()) +
  labs(
    title = "Departament where the hospital/laboratory is located",
    subtitle = "Percentage and frequency (N = 47)",
    x = NULL,
    y = NULL
  ) +
  coord_flip() +
...
### Time the laboratory has been carrying out mycological diagnoses by region
...
### Time the laboratory has been carrying out mycological diagnoses by capacity

```{r}
survey %>% 
  select(years_myco_diag_lab, capacity) %>% 
  tbl_summary(
    by = capacity,
    label = years_myco_diag_lab ~ "Time the laboratory has been carrying out mycological diagnoses",
    statistic = years_myco_diag_lab ~ c(
...
### Total number of hospital beds by capacity 
...
services_hosp_lab <- survey %>% 
  ...
  mutate(
    services_hosp_lab = factor(
      ...
      labels = c(
        "COVID-19 Area", "Oncohematology", "Infectology", "PROCETSS",
...
services_hosp_lab_plot <- services_hosp_lab %>% 
  ...
  labs(
    ...
    subtitle = "Percentage by service (N = 47)",
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
equip_fungal_diag_plot <- equip_fungal_diag %>% 
  ...
  labs(
    ...
    subtitle = "Percentage by equipment (N = 47)",
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
## Technical staff in the microbiology laboratory by region

```{r}
survey %>% 
  select(n_tech_staff_lab, flg_lima) %>% 
  mutate(n_tech_staff_lab = ifelse(n_tech_staff_lab > 100, NA, n_tech_staff_lab)) %>% 
  tbl_summary(
    by = flg_lima,
    label = n_tech_staff_lab ~ "Technical staff in the microbiology laboratory",
...
## Technical staff in the microbiology laboratory by capacity

```{r}
survey %>% 
  select(n_tech_staff_lab, capacity) %>% 
  mutate(n_tech_staff_lab = ifelse(n_tech_staff_lab > 100, NA, n_tech_staff_lab)) %>% 
  tbl_summary(
    by = capacity,
    label = n_tech_staff_lab ~ "Technical staff in the microbiology laboratory",
...
### Professional staff in the microbiology laboratory by region

```{r}
survey %>% 
  select(n_profe_staff_lab, flg_lima) %>% 
  mutate(n_profe_staff_lab = ifelse(n_profe_staff_lab > 100, NA, n_profe_staff_lab)) %>% 
  tbl_summary(
    by = flg_lima,
    label = n_profe_staff_lab ~ "Professional staff in the microbiology laboratory",
...
### Professional staff in the microbiology laboratory by capacity

```{r}
survey %>% 
  select(n_profe_staff_lab, capacity) %>% 
  mutate(n_profe_staff_lab = ifelse(n_profe_staff_lab > 100, NA, n_profe_staff_lab)) %>% 
  tbl_summary(
    by = capacity,
    label = n_profe_staff_lab ~ "Professional staff in the microbiology laboratory",
...
### Total staff in the microbiology laboratory by region

```{r}
survey %>% 
  select(total_n_staff, flg_lima) %>% 
  mutate(total_n_staff = ifelse(total_n_staff > 100, NA, total_n_staff)) %>% 
  tbl_summary(
    by = flg_lima,
    label = total_n_staff ~ "Total staff in the microbiology laboratory",
...
### Total staff in the microbiology laboratory by capacity

```{r}
survey %>% 
  select(total_n_staff, capacity) %>% 
  mutate(total_n_staff = ifelse(total_n_staff > 100, NA, total_n_staff)) %>% 
  tbl_summary(
    by = capacity,
    label = total_n_staff ~ "Total staff in the microbiology laboratory",
...
### Total staff in the mycology area by region

```{r}
survey %>% 
  select(total_n_staff_mico_lab, flg_lima) %>% 
  mutate(
    total_n_staff_mico_lab = ifelse(
      total_n_staff_mico_lab > 100, NA, total_n_staff_mico_lab
    )
...
### Total staff in the mycology area by capacity

```{r}
survey %>% 
  select(total_n_staff_mico_lab, capacity) %>% 
  mutate(
    total_n_staff_mico_lab = ifelse(
      total_n_staff_mico_lab > 100, NA, total_n_staff_mico_lab
    )
...
staff_myco_lab_train_plot <- staff_myco_lab_train %>% 
  ...
  labs(
    ...
    subtitle = "Percentage by staff type (N = 2)",
    x = NULL,
    y = NULL,
    fill = "Aimed at?"
  ) +
...
hosp_units_samples <- survey %>% 
  ...
  mutate(
    hosp_units_samples_myco = factor(
      ...
      labels = c(
        "Hosp.: Medicine", "Hosp.: Gynecology", 
        "Hosp.: Surgery", "Hosp.: Pediatrics", 
        "Hosp.: Infectology", "ICU", 
        "Outpatient: Infectology", "Outpatient: Dermatology", 
        "Outpatient: Gynecology", "Outpatient: Pneumology", 
        "Oncology", "Transplants", "Others"
      )
    ),
    availability = factor(
      availability, levels = c("No seleccionados", "Seleccionados"),
...
hosp_units_samples_plot <- hosp_units_samples %>% 
  ...
  labs(
    ...
    subtitle = "Percentage by departments (N = 47)",
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
most_freq_sample_plot <- most_freq_sample %>% 
  ...
  labs(
    ...
    subtitle = "Percentage by sample (N = 47)",
    x = NULL,
    y = NULL,
    fill = "Most frequent?"
  ) +
...
micro_tests_plot <- micro_tests %>% 
  ...
  labs(
    ...
    subtitle = "Percentage by test (N = 47)",
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
delivery_time_china_plot <- delivery_time_china %>% 
  ...
  labs(
    ...
    subtitle = "Percentage and frequency (N = 37)",
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
delivery_time_giemsa_plot <- delivery_time_giemsa %>% 
  ...
  labs(
    ...
    subtitle = "Percentage and frequency (N = 30)",
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
delivery_methods_plot <- delivery_methods %>% 
  ...
  labs(
    ...
    subtitle = "Percentage by sample (N = 44)",
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
culture_media_yeast_plot <- culture_media_yeast %>% 
  ...
  labs(
    ...
    subtitle = sprintf("Percentage by sample (N = %s)", mean(culture_media_yeast$total)),
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
flg_identi_yeast_plot <- flg_identi_yeast %>% 
  ...
  labs(
    ...
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_identi_yeast$total)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
identi_yeast_plot <- identi_yeast %>% 
  ...
  labs(
    ...
    subtitle = sprintf("Percentage by sample (N = %s)", mean(identi_yeast$total, na.rm = TRUE)),
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
flg_store_yeast_plot <- flg_store_yeast %>% 
  ...
  labs(
    ...
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_store_yeast$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
culture_media_mold_plot <- culture_media_mold %>% 
  ...
  labs(
    ...
    subtitle = sprintf("Percentage by culture media (N = %s)", mean(culture_media_mold$total, na.rm = TRUE)),
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
flg_identi_mold_plot <- flg_identi_mold %>% 
  ...
  labs(
    ...
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_identi_mold$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
flg_identif_mold_ext_plot <- flg_identif_mold_ext %>% 
  ...
  labs(
    ...
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_identif_mold_ext$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
identi_mold <- survey %>% 
  ...
  mutate(identi_mold = fct_rev(identi_mold)) %>% 
  group_by(identi_mold, availability) %>% 
  summarise(n = n()) %>% 
  mutate(total = sum(n), prop = n / total) %>% 
  mutate(text = str_c(round(100 * prop, 2), "%")) %>% 
  ungroup() %>% 
  complete(identi_mold, availability) %>% 
  replace_na(list(n = 0, prop = 0, text = "0%"))

identi_mold_prop <- identi_mold %>% 
  filter(availability == "Yes") %>% 
  pull(prop)

identi_mold <- identi_mold %>% 
  mutate(
    identi_mold = factor(
      identi_mold,
      levels = levels(identi_mold)[order(identi_mold_prop, decreasing = TRUE)]
    )
  ) %>% 
  mutate(identi_mold = fct_rev(identi_mold))
```
...
identi_mold_plot <- identi_mold %>% 
  ggplot(aes(x = identi_mold, y = prop, fill = availability)) +
  geom_col(position = "stack") +
  scale_y_continuous(limits = c(0, 1), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  theme(panel.grid.minor.x = element_blank()) +
  labs(
    ...
    subtitle = sprintf("Percentage by system (N = %s)", mean(identi_mold$total, na.rm = TRUE)),
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
flg_store_mold <- survey %>% 
  ...
  mutate(
    flg_store_mold_isolates = fct_explicit_na(
      flg_store_mold_isolates, na_level = "No response"
    )
  )
```
...
flg_store_mold_plot <- flg_store_mold %>% 
  ggplot(aes(reorder(flg_store_mold_isolates, prop), prop)) +
  geom_col(fill = "#01324f") +
  geom_text(aes(label = text), hjust = -0.1, size = 3.2) +
  scale_y_continuous(limits = c(0, 1.03), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  theme(panel.grid.minor.x = element_blank()) +
  labs(
    title = "Does the laboratory routinely stock clinical mold isolates?",
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_store_mold$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
flg_suscep_tests <- survey %>% 
  ...
  mutate(
    flg_antifungal_suscep_tests = fct_explicit_na(
      flg_antifungal_suscep_tests, na_level = "No response"
    )
  )
```
...
flg_suscep_tests_plot <- flg_suscep_tests %>% 
  ggplot(aes(reorder(flg_antifungal_suscep_tests, prop), prop)) +
  geom_col(fill = "#01324f") +
  geom_text(aes(label = text), hjust = -0.1, size = 3.2) +
  scale_y_continuous(limits = c(0, 0.69), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  theme(panel.grid.minor.x = element_blank()) +
  labs(
    title = "Does the laboratory offer in-vitro antifungal susceptibility testing services?",
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_suscep_tests$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
flg_suscep_tests_lab <- survey %>% 
  ...
  mutate(
    flg_antifungal_suscep_tests_lab = factor(
      ...
      labels = c("No, they are sent to\nan external laboratory", "Yes")
    )
  ) %>% 
  mutate(
    flg_antifungal_suscep_tests_lab = fct_explicit_na(
      flg_antifungal_suscep_tests_lab, na_level = "No response"
    )
  )
```
...
flg_suscep_tests_lab_plot <- flg_suscep_tests_lab %>% 
  ggplot(aes(reorder(flg_antifungal_suscep_tests_lab, prop), prop)) +
  geom_col(fill = "#01324f") +
  geom_text(aes(label = text), hjust = -0.1, size = 3.2) +
  scale_y_continuous(limits = c(0, 1.05), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  ...
  labs(
    ...
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_suscep_tests_lab$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
type_antifungal <- survey %>% 
  ...
  mutate(type_antifungal = fct_rev(type_antifungal)) %>% 
  group_by(type_antifungal, availability) %>% 
  summarise(n = n()) %>% 
  mutate(total = sum(n), prop = n / total) %>% 
  mutate(text = str_c(round(100 * prop, 2), "%")) %>% 
  ungroup() %>% 
  complete(type_antifungal, availability) %>% 
  replace_na(list(n = 0, prop = 0, text = "0%"))

type_antifungal_prop <- type_antifungal %>% 
  filter(availability == "Yes") %>% 
  pull(prop)

type_antifungal <- type_antifungal %>% 
  mutate(
    type_antifungal = factor(
      type_antifungal,
      levels = levels(type_antifungal)[order(type_antifungal_prop, decreasing = TRUE)]
    )
  ) %>% 
  mutate(type_antifungal = fct_rev(type_antifungal))
```
...
type_antifungal_plot <- type_antifungal %>% 
  ggplot(aes(x = type_antifungal, y = prop, fill = availability)) +
  geom_col(position = "stack") +
  scale_y_continuous(limits = c(0, 1), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  theme(panel.grid.minor.x = element_blank()) +
  labs(
    ...
    subtitle = sprintf("Percentage by test (N = %s)", mean(type_antifungal$total, na.rm = TRUE)),
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
...
flg_antibodies_tests <- survey %>% 
  ...
  mutate(
    flg_antifungal_antibodies_tests_lab = fct_explicit_na(
      flg_antifungal_antibodies_tests_lab, na_level = "No response"
    )
  )
```
...
flg_antibodies_tests_plot <- flg_antibodies_tests %>% 
  ggplot(aes(reorder(flg_antifungal_antibodies_tests_lab, prop), prop)) +
  geom_col(fill = "#01324f") +
  geom_text(aes(label = text), hjust = -0.1, size = 3.2) +
  scale_y_continuous(limits = c(0, 1.07), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  ...
  labs(
    ...
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_antibodies_tests$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
flg_antibodies_tests_other <- survey %>% 
  ...
  mutate(
    flg_antifungal_antibodies_tests_other_lab = fct_explicit_na(
      flg_antifungal_antibodies_tests_other_lab, na_level = "No response"
    )
  )
```
...
flg_antibodies_tests_other_plot <- flg_antibodies_tests_other %>% 
  ggplot(aes(reorder(flg_antifungal_antibodies_tests_other_lab, prop), prop)) +
  geom_col(fill = "#01324f") +
  geom_text(aes(label = text), hjust = -0.1, size = 3.2) +
  scale_y_continuous(limits = c(0, 1.05), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  ...
  labs(
    ...
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_antibodies_tests_other$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
flg_antibodies_tests_ext <- survey %>% 
  ...
  mutate(
    flg_antifungal_antibodies_tests_ext_lab = fct_explicit_na(
      flg_antifungal_antibodies_tests_ext_lab, na_level = "No response"
    )
  )
```
...
flg_antibodies_tests_ext_plot <- flg_antibodies_tests_ext %>% 
  ggplot(aes(reorder(flg_antifungal_antibodies_tests_ext_lab, prop), prop)) +
  geom_col(fill = "#01324f") +
  geom_text(aes(label = text), hjust = -0.1, size = 3.2) +
  ...
  theme(legend.position = "none", panel.grid.minor.x = element_blank()) +
  labs(
    title = "Are the tests for the detection of antibodies against fungi sentto an\nexternal laboratory?",
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_antibodies_tests_ext$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
flg_crypto_antigens <- survey %>% 
  ...
  mutate(
    flg_crypto_antigens_test_lab = fct_explicit_na(
      flg_crypto_antigens_test_lab, na_level = "No response"
    )
  )
```
...
flg_crypto_antigens_plot <- flg_crypto_antigens %>% 
  ggplot(aes(reorder(flg_crypto_antigens_test_lab, prop), prop)) +
  geom_col(fill = "#01324f") +
  geom_text(aes(label = text), hjust = -0.1, size = 3.2) +
  ...
  labs(
    ...
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_crypto_antigens$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
## Are *Cryptococcus* antigen tests available at another hospital lab?
...
flg_crypto_antigens_other <- survey %>% 
  ...
  mutate(
    flg_crypto_antigens_test_other_lab = fct_explicit_na(
      flg_crypto_antigens_test_other_lab, na_level = "No response"
    )
  )
```
...
flg_crypto_antigens_other_plot <- flg_crypto_antigens_other %>% 
  ggplot(aes(reorder(flg_crypto_antigens_test_other_lab, prop), prop)) +
  geom_col(fill = "#01324f") +
  geom_text(aes(label = text), hjust = -0.1, size = 3.2) +
  scale_y_continuous(limits = c(0, 0.98), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  theme(legend.position = "none", panel.grid.minor.x = element_blank()) +
  labs(
    title = "Are Cryptococcus antigen tests available at another hospital lab?",
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_crypto_antigens_other$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
```{r}
flg_crypto_antigens_ext <- survey %>% 
  ...
  mutate(
    flg_crypto_antigens_test_ext_lab = fct_explicit_na(
      flg_crypto_antigens_test_ext_lab, na_level = "No response"
    )
  )
```

```{r}
flg_crypto_antigens_ext_plot <- flg_crypto_antigens_ext %>% 
  ggplot(aes(reorder(flg_crypto_antigens_test_ext_lab, prop), prop)) +
  geom_col(fill = "#01324f") +
  geom_text(aes(label = text), hjust = -0.1, size = 3.2) +
  scale_y_continuous(limits = c(0, 0.81), expand = c(0, 0), labels = scales::percent) +
  theme_minimal() +
  ...
  labs(
    title = "Are Cryptococcus antigen tests available at another hospital lab?",
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(flg_crypto_antigens_ext$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
crypto_plot <- crypto %>% 
  ...
  labs(
    ...
    subtitle = sprintf("Percentage and frequency (N = %s)", mean(crypto$total, na.rm = TRUE)),
    x = NULL,
    y = NULL
  ) +
  coord_flip()
...
flg_pcr_tests_plot <- flg_pcr_tests %>% 
  ...
  labs(
    ...
    subtitle = "Percentage (%) and frecuency (n) of laboratories from total (N = 47)",
    x = NULL,
    y = NULL
  ) +
  scale_fill_npg() +
...
antifungal_plot <- antifungal %>% 
  ...
  labs(
    ...
    subtitle = "Availability percentage by antifungal type from total (N = 47)",
    x = NULL,
    y = NULL,
    fill = "Available?"
  ) +
